...
public class ANN{
	...
	public double alpha;
	List<LayerNN> layers = new List<LayerNN>();
	...
	public ANN (int nI, int nO, int nH, int nPH, double a)
	{
		...
		if(numHidden > 0)
		{
			layers.Add(new LayerNN(numNPerHidden, numInputs));

			for(int i = 0; i < numHidden-1; i++)
			{
				layers.Add(new LayerNN(numNPerHidden, numNPerHidden));
			}

			layers.Add(new LayerNN(numOutputs, numNPerHidden));
		}
		else
		{
			layers.Add(new LayerNN(numOutputs, numInputs));
		}
	}
